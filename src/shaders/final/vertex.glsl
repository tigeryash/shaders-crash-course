...
float fit(float unscaled, float minIn, float maxIn, float minOut, float maxOut) {
    return (unscaled - minIn)* (maxOut - minOut) /(maxIn - minIn)  + minOut;
}
...
void main(){
    ...
    //MVP
    float newDisplacement = vDisplacement * 3.0;
    vec3 newPosition = position + normal *newDisplacement;
